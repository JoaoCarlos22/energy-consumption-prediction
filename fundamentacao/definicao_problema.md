...
### 2.1 Machine Learning em Consumo de Energia

A previsão do consumo de energia tem passado por uma transição significativa dos modelos físicos tradicionais para as abordagens guiadas por dados (data-driven), impulsionada pela necessidade de lidar com interações altamente complexas. Conforme Wu et al. (2024), a modelagem baseada em física de edifícios exige informações minuciosas das instalações, o que resulta em lentidão e alto custo computacional, tornando o uso de algoritmos de Machine Learning uma alternativa superior e amplamente adotada. Zhao et al. (2025) corroboram essa visão, destacando que os modelos preditivos lineares tradicionais falham em capturar as características não-lineares inerentes às oscilações dos dados de eletricidade atuais . Adicionalmente, Lee et al. (2022) ressaltam que a alta precisão preditiva fornecida por modelos baseados em inteligência artificial — como Redes Neurais Artificiais (ANN) e Sistemas de Inferência Neuro-Fuzzy (ANFIS) — é crucial para adequar a demanda global à oferta, evitando o superávit ou a escassez no fornecimento elétrico e contribuindo diretamente para a redução das emissões de gases de efeito estufa.

No que tange ao desempenho algorítmico, a literatura atual evidencia o grande destaque de métodos baseados em árvores de decisão e arquiteturas preditivas híbridas. Sarswatula et al. (2022) demonstraram em suas extensas análises que o algoritmo Random Forest Regressor obteve a melhor performance e coeficiente de determinação para prever a pegada energética no robusto setor da manufatura industrial. Para elevar ainda mais esses níveis de precisão, Ding et al. (2022) comprovaram empiricamente que a integração prévia de algoritmos de clusterização (agrupamento) aos modelos de regressão — em especial ao Extreme Gradient Boosting (XGBoost) e ao Support Vector Regression (SVR) — aumenta de maneira substancial a acurácia para previsões de curto prazo, visto que a técnica permite que o modelo de Machine Learning seja treinado com base em subconjuntos de dados muito mais homogêneos e padronizados.
